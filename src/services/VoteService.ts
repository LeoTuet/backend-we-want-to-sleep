...
export class VoteService {
  ...
  public async checkIfAlreadyVoted(ballotID: string, token: string): Promise<boolean> {
    return (await this.getVote(ballotID, token)) != null
  }

  public async checkIfVoteOptionValid(ballotID: string, vote: string): Promise<boolean> {
    return (await BallotRepository.getBallot(ballotID)).options.includes(vote)
  }
}